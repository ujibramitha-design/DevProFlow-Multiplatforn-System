...
export class DateCalculator {
     ...
     */
    static calculateAging(startDate, endDate = new Date()) {
     ...
     */
    static calculateBusinessDays(startDate, endDate, config) {
        ...
        while (currentDate <= endDate) {
            ...
            if (workingConfig.workingDays.includes(dayOfWeek)) {
                ...
                const isHoliday = workingConfig.holidays.some(holiday => this.isSameDay(holiday, currentDate));
                if (!isHoliday) {
                    businessDays++;
                }
            }
     ...
     */
    static addWorkingDays(startDate, days, config) {
        const workingConfig = { ...this.WORKING_DAYS_CONFIG, ...config };
        ...
        while (daysAdded < days) {
            ...
            if (workingConfig.workingDays.includes(dayOfWeek)) {
                const isHoliday = workingConfig.holidays.some(holiday => this.isSameDay(holiday, result));
                if (!isHoliday) {
                    daysAdded++;
                }
            }
     ...
     */
    static getDueDate(startDate, agingDays) {
        const start = typeof startDate === 'string' ? new Date(startDate) : startDate;
     ...
     */
    static formatIndonesian(date, includeTime = false) {
     ...
     */
    static isWorkingDay(date, config) {
        const workingConfig = { ...this.WORKING_DAYS_CONFIG, ...config };
        const dayOfWeek = date.getDay();
        if (!workingConfig.workingDays.includes(dayOfWeek)) {
            return false;
        }
        const isHoliday = workingConfig.holidays.some(holiday => this.isSameDay(holiday, date));
        return !isHoliday;
    }
    /**
     * Get next working day
     */
    static getNextWorkingDay(date, config) {
        const workingConfig = { ...this.WORKING_DAYS_CONFIG, ...config };
     ...
     */
    static getPreviousWorkingDay(date, config) {
        const workingConfig = { ...this.WORKING_DAYS_CONFIG, ...config };
     ...
     */
    static calculateSLADeadline(startDate, slaDays, config) {
        const start = typeof startDate === 'string' ? new Date(startDate) : startDate;
     ...
     * Check if SLA is met
     */
    static isSLAMet(startDate, endDate, slaDays, config) {
        const start = typeof startDate === 'string' ? new Date(startDate) : startDate;
        const end = typeof endDate === 'string' ? new Date(endDate) : endDate;
     ...
     * Get SLA status
     */
    static getSLAStatus(startDate, endDate = new Date(), slaDays, config) {
        const start = typeof startDate === 'string' ? new Date(startDate) : startDate;
        const end = typeof endDate === 'string' ? new Date(endDate) : endDate;
        const businessDays = this.calculateBusinessDays(start, end, config);
...
DateCalculator.INDONESIAN_TIMEZONE = 'Asia/Jakarta';
DateCalculator.WORKING_DAYS_CONFIG = {
    startHour: 9,
    endHour: 17,
